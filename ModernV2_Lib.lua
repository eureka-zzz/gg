...
function ModernV2:AnimateTextGradients(dt)
	...
	for i = #ModernV2.TextGradientObjects, 1, -1 do
		local Gradient = ModernV2.TextGradientObjects[i];
		if Gradient and Gradient.Parent then
			...
			table.remove(ModernV2.TextGradientObjects, i);
		end;
	end;
end;
...
function ModernV2:ApplyLock(Frame, isLocked, lockMessage)
	...
	local function BuildOverlay()
		...
		local lockTweenIn = TweenInfo.new(0.08);
		local lockTweenOut = TweenInfo.new(0.12);
		ModernV2:AddSignal(LockOverlay.MouseButton1Click:Connect(function()
			...
			FeedbackBusy = true;
			ModernV2.PlayAnimate(LockOverlay, lockTweenIn, {
				BackgroundTransparency = 0.18,
			});

			task.delay(0.12, function()
				if LockOverlay and LockOverlay.Parent then
					ModernV2.PlayAnimate(LockOverlay, lockTweenOut, {
...
function ModernV2:CreateShadow(parent , RollingEffect)
	...
	Shadow.Render = LPH_NO_VIRTUALIZE(function(self , value)
		...
		if value then
			...
			if RollingEffect and not ModernV2.LowEndMode then
				Shadow.RollingEffectThread = RunService.Heartbeat:Connect(function(dt)
					if parent and not parent.Visible then return end;
					local deltaRotation = 20 * (dt * 30);
					r1.Rotation = r1.Rotation + deltaRotation;
					r2.Rotation = r2.Rotation + deltaRotation;
					r3.Rotation = r3.Rotation + deltaRotation;
					r4.Rotation = r4.Rotation + deltaRotation;
				end);
			end;
		else
			ModernV2.PlayAnimate(UIShadowSafe85 , SlowyTween , {
...
function ModernV2:RegisiterHandler(Handler , Signal)
	...
	function handle:AddDropdown(Config)
		...
		function DropdownLib:Destroy()
			if RefreshSignal then RefreshSignal:Disconnect() end
			if SecureSignal then SecureSignal:Disconnect() end
			if BlockRoot then BlockRoot:Destroy() end
			ModernV2.SpecialDropdowns[DropdownLib] = nil;
...
function ModernV2:AddTooltip(HoverInstance, InfoStr, DisabledInfoStr)
	...
	local function DoHover()
		...
		repeat
			...
			if Mouse.X ~= (TooltipCurrentMouseX or -1) or Mouse.Y ~= (TooltipCurrentMouseY or -1) then
				TooltipCurrentMouseX = Mouse.X;
				TooltipCurrentMouseY = Mouse.Y;
...
function ModernV2:RegisiterItem(Frame , Signel)
	...
	function idx:AddButton(Config)
		...
		local function BuildButton(BtnConfig)
			...
			Button.SetRender = LPH_NO_VIRTUALIZE(function(value)
				local bgTrans = 1;
				local labelTrans = value and 0.200 or 1;
				local lineTrans = value and 0.650 or 1;
				local iconTrans = value and 0.250 or 1;
				ModernV2.PlayAnimate(ButtonFrame, SlowyTween, {BackgroundTransparency = bgTrans});
				ModernV2.PlayAnimate(BasedLabel, SlowyTween, {TextTransparency = labelTrans});
				ModernV2.PlayAnimate(LineFrame, SlowyTween, {BackgroundTransparency = lineTrans});
				ModernV2.PlayAnimate(Icon, SlowyTween, {TextTransparency = iconTrans});
			end);
	...
	function idx:AddViewport(Config)
		...
		-- changed to use next instead of pairs
		...
		local function StartRender()
			...
			RenderConn = ModernV2:AddSignal(RunService.RenderStepped:Connect(LPH_NO_VIRTUALIZE(function(dt)
				...
				if Config.AutoRotate then
					...
					if LastTick < 1/30 then return end;
					dt = LastTick;
					LastTick = 0;
				end

				local Alpha = math.clamp((dt or (1 / 60)) * 14, 0.08, 0.9);
...
local MinimizeTween = TweenInfo.new(0.3, Enum.EasingStyle.Quint, Enum.EasingDirection.Out)
local SidebarTween = TweenInfo.new(0.25, Enum.EasingStyle.Quint, Enum.EasingDirection.Out)
local BadgeTween = TweenInfo.new(0.3, Enum.EasingStyle.Back, Enum.EasingDirection.Out)
	...
	local function EnableHeaderRunningText(Label, Speed, Gap)
		...
		ModernV2:AddSignal(RunService.RenderStepped:Connect(function()
			if not Overflow or not Label.Parent or not Clip.Parent or not Clip.Visible then
				return;
			end;

			local Cycle = TextWidth + Gap;
			local X = -(((os.clock() - StartTick) * Speed) % Cycle);
	...
	do
		local Input = ModernV2:CreateInput(CloseButton , LPH_NO_VIRTUALIZE(function()
			...
			ModernV2.PlayAnimate(WindowFrame, MinimizeTween, {
				Position = UDim2.fromScale(0.5, 0.5),
			});
		...
		function Window:SetMinimized(State)
			...
			if State then
				...
				if Window.__ContainerStrips then
					for _, StripEntry in next, Window.__ContainerStrips do
				...
				ModernV2.PlayAnimate(RightMenuFrame, MinimizeTween, {
	...
	do
		...
		local function SetSidebarExpanded(State)
			...
			ModernV2.PlayAnimate(LeftMenuFrame, SidebarTween, {
				Size = UDim2.new(0, TargetOffset, 1, 0),
			});

			ModernV2.PlayAnimate(RightMenuFrame, SidebarTween, {
			...
			if Window.SidebarLabelRegistry then
				for _, Refresh in next, Window.SidebarLabelRegistry do
					Refresh();
				end;
			end;
		end;
	...
	function Window:CreateHomeTab(Config)
		...
		if Config.AutoSetup ~= false then
			...
			if typeof(Config.Changelog) == "table" and #Config.Changelog > 0 then
				...
				for Index,Entry in next, Config.Changelog do
					if Index > 4 then
						break;
					end;
			...
			local function StartHomeUpdate()
				if not HomeSignal then HomeSignal = ModernV2:AddSignal(RunService.Heartbeat:Connect(UpdateHome)) end
			end
			local function StopHomeUpdate()
				if HomeSignal then HomeSignal:Disconnect(); HomeSignal = nil end
			end
		...
		if typeof(Config.Buttons) == "table" then
			for _,ButtonConfig in next, Config.Buttons do
	...
	function Window:AddTab(Config)
		...
		function Tab:SetBadge(visible)
			...
			if BadgeVisible then
				UpdateBadgeSize();
				ModernV2.PlayAnimate(BadgeDot, BadgeTween, {
					BackgroundTransparency = 0,
				});
				if BadgeCountValue and BadgeCountValue > 0 then
		...
		function Tab:SetLocked(value)
			...
			if value == true and Window.Tabs[Window.CurrentTab] == Tab then
				...
				for Index,OtherTab in next, Window.Tabs do
		...
		function Tab:AddTabbox(Config)
			...
			function Tabbox:AddTab(Name , IconName)
				...
				function SubTab:Show()
					...
					for _,Item in next, Tabbox.Tabs do
		...
		function Tab:AddContainerTab(ContainerConfig)
			...
			local function SelectContainerTab()
				...
				for _, Entry in next, Host.Items do
	...
	function Window:_InitConfig()
		...
		task.delay(1,function()
			...
			if Window.ConfigAutoSave then
				task.spawn(function()
					while not Window.Destroyed do task.wait(5.75);
	...
	local reciveSignal;
	table.insert(Window.OnDestroyCallbacks, function() if reciveSignal then reciveSignal:Disconnect() reciveSignal = nil end end)
...
local dragCon1, dragCon2
dragCon1 = header.InputBegan:Connect(function(input)
if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
dragging = true
dragStart = input.Position
...
dragInput = input
local dragCon3
dragCon3 = input.Changed:Connect(function()
if input.UserInputState == Enum.UserInputState.End then
if dragInput == input then dragging = false dragInput = nil end
if dragCon3 then dragCon3:Disconnect() end
end
end)
end
end)
dragCon2 = UserInputService.InputChanged:Connect(function(input)
...
if dragCon1 then dragCon1:Disconnect() end
if dragCon2 then dragCon2:Disconnect() end
end)
end
end
...
local function ShowLoadingScreen(onComplete)
    ...
    local trailTweenInfo = TweenInfo.new(0.12, Enum.EasingStyle.Quad)
local function animateTrails()
while animationRunning do
for _, trail in ipairs(trails) do
local newWidth = trail.config.width + math.random(-12, 12)
TweenService:Create(trail.frame, trailTweenInfo, {Size = UDim2.new(0, newWidth, 0, mobile and 2 or 3), BackgroundTransparency = 0.1 + math.random() * 0.3}):Play()
end
task.wait(0.1)
end
end

    local shipTweenInfo = TweenInfo.new(0.04, Enum.EasingStyle.Linear)
local function animateShipShake()
while animationRunning do
local shakeAmount = mobile and 2 or 3
TweenService:Create(shipContainer, shipTweenInfo, {Position = UDim2.new(0.5, math.random(-shakeAmount, shakeAmount), 0.35, math.random(-1, 1))}):Play()
task.wait(0.04)
end
end
    ...
    task.spawn(function()
        ...
        for _, trail in next, trails do TweenService:Create(trail.frame, fadeInfo, {BackgroundTransparency = 1}):Play() end
        for _, line in next, longLines do TweenService:Create(line, fadeInfo, {BackgroundTransparency = 1}):Play() end
        for i = 1, 5 do
...
local function BuildKeylessUI()
    ...
    local hoverTweenInfo = TweenInfo.new(0.15)
    closeBtn.MouseEnter:Connect(function() TweenService:Create(closeBtn, hoverTweenInfo, {ImageColor3 = Modern.Theme.Error}):Play() end)
    closeBtn.MouseLeave:Connect(function() TweenService:Create(closeBtn, hoverTweenInfo, {ImageColor3 = Modern.Theme.TextDim}):Play() end)
...
spinDt = spinDt + dt
if spinDt < 0.016 then return end
local frameDt = spinDt
spinDt = 0
if statusIcon and statusIcon.Parent then statusIcon.Rotation = (statusIcon.Rotation + frameDt * 360) % 360
    ...
    OpenProviderDropdown = function()
        ...
        local Names = {}
        for k in next, Modern.Provider do table.insert(Names, k) end
        table.sort(Names)
...
function ModernV2:AddLoading(Config)
	...
	local StepProgressTween = TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
function Loading:SetCurrentStep(Step)
		...
		ModernV2.PlayAnimate(BarFill , StepProgressTween , {
